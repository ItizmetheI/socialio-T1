...
export default function PricingCard({ service }: React.PropsWithChildren<{ service: any }>) {
  ...
  return (
    <motion.div
      ...
      className={`group flex flex-col bg-surface-container border ${service.popular ? 'border-primary/30 shadow-[0_0_30px_rgba(var(--color-primary-rgb),0.15)]' : 'border-white/5'} rounded-3xl p-8 transition-colors relative overflow-hidden text-left shadow-lg h-full`}
    >
      {service.popular && (
